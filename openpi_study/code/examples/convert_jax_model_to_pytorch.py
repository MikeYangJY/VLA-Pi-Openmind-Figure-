...
# 定位：权重转换｜读取Orbax/JAX权重并重排维度与名称，生成PyTorch可加载的模型权重。
# 阅读顺序：先convert_pi0_checkpoint看总体路径，再看PaliGemma/Gemma切片与转置规则。
# 重点边界：转换权重不是重新训练；同名层的矩阵轴约定可能不同，必须按形状验证。
...
# 【slice_paligemma_state_dict】本函数位于“权重转换”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 state_dict.pop(jax_key).transpose → state_dict.pop → state_dict.pop(jax_key).reshape 追踪具体实现。
# 输入接口：state_dict；config。
# 返回值可从这里追踪：(final_state_dict, expert_dict)。
# 内部调用线索：state_dict.pop(jax_key).transpose → state_dict.pop → state_dict.pop(jax_key).reshape → encoderblock_layernorm0_scale[i].transpose → encoderblock_layernorm1_scale[i].transpose（含分支中的调用，实际路径由条件决定）。
...
# 【slice_gemma_state_dict】本函数位于“权重转换”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 hasattr → state_dict.pop → llm_attention_q_einsum[i].transpose(0, 2, 1).reshape 追踪具体实现。
# 输入接口：state_dict；config；num_expert；checkpoint_dir；pi05。
# 返回值可从这里追踪：final_state_dict。
# 内部调用线索：hasattr → state_dict.pop → llm_attention_q_einsum[i].transpose(0, 2, 1).reshape → llm_attention_q_einsum[i].transpose → llm_attention_kv_einsum[i, 0, 0].transpose（含分支中的调用，实际路径由条件决定）。
...
# 【slice_initial_orbax_checkpoint】本函数位于“权重转换”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 openpi.models.model.restore_params → traversals.flatten_mapping 追踪具体实现。
# 输入接口：checkpoint_dir:str；restore_precision:str | None。
# 返回值可从这里追踪：{'paligemma_params': traversals.flatten_mapping(params['PaliGemma'], sep='/'), 'projection_params': params}。
# 内部调用线索：openpi.models.model.restore_params → traversals.flatten_mapping（含分支中的调用，实际路径由条件决定）。
...
# 【load_jax_model_and_print_keys】本函数位于“权重转换”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 checkpoint_dir.startswith → os.path.abspath → ocp.PyTreeCheckpointer 追踪具体实现。
# 输入接口：checkpoint_dir:str。
# 内部调用线索：checkpoint_dir.startswith → os.path.abspath → ocp.PyTreeCheckpointer → checkpointer.metadata → print（含分支中的调用，实际路径由条件决定）。
...
# 【convert_pi0_checkpoint】本函数位于“权重转换”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 print → slice_initial_orbax_checkpoint → isinstance 追踪具体实现。
# 输入接口：checkpoint_dir:str；precision:str；output_path:str；model_config:openpi.models.pi0_config.Pi0Config。
# 内部调用线索：print → slice_initial_orbax_checkpoint → isinstance → torch.from_numpy → np.array（含分支中的调用，实际路径由条件决定）。
def convert_pi0_checkpoint(
    ...
    # 【convert_pi0_checkpoint.PaliGemmaConfig】读取Orbax/JAX权重并重排维度与名称，生成PyTorch可加载的模型权重。 本类封装其中的配置、状态或接口；具体入口见下面的方法。
    class PaliGemmaConfig:
        # 【convert_pi0_checkpoint.PaliGemmaConfig.__init__】初始化PaliGemmaConfig的依赖和内部状态；构造对象本身与后续执行/训练要区分。主要保存：self.vision_config、self.text_config。
        # 内部调用线索：type('obj', (object,), {'hidden_size': 1152, 'num_hidden_layers': 27, … → type → type('obj', (object,), {'hidden_size': 2048, 'num_hidden_layers': 18, …（含分支中的调用，实际路径由条件决定）。
        def __init__(self):
...
# 【main】本脚本入口：读取Orbax/JAX权重并重排维度与名称，生成PyTorch可加载的模型权重。 先convert_pi0_checkpoint看总体路径，再看PaliGemma/Gemma切片与转置规则。
# 输入接口：checkpoint_dir:str；config_name:str；output_path:str | None；precision:Literal['float32', 'bfloat16', 'float16']；inspect_only:bool。
# 内部调用线索：_config.get_config → isinstance → ValueError → load_jax_model_and_print_keys → print（含分支中的调用，实际路径由条件决定）。
def main(
    checkpoint_dir: str,
    config_name: str,
