...
# 定位：资源加载｜根据本地路径或远程URI查找资源、使用缓存与文件锁，并处理失效缓存。
# 阅读顺序：maybe_download是入口；按协议选下载方式，再检查权限与时间信息。
# 重点边界：加载策略可能触发大权重下载；源码阅读不需要调用它。
...
# 【get_cache_dir】本函数位于“资源加载”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 pathlib.Path(os.getenv(_OPENPI_DATA_HOME, DEFAULT_CACHE_DIR)).expandus… → pathlib.Path → os.getenv 追踪具体实现。
# 返回类型：pathlib.Path；类型/shape约定需与调用方配套。
# 内部调用线索：pathlib.Path(os.getenv(_OPENPI_DATA_HOME, DEFAULT_CACHE_DIR)).expandus… → pathlib.Path → os.getenv → cache_dir.mkdir → _set_folder_permission（含分支中的调用，实际路径由条件决定）。
...
# 【maybe_download】本地路径直接使用；远程路径经过缓存与文件锁后下载，避免多个进程重复写入。
# 输入接口：url:str；force_download:bool。
# 返回类型：pathlib.Path；类型/shape约定需与调用方配套。
# 内部调用线索：urllib.parse.urlparse → pathlib.Path → path.exists → FileNotFoundError → path.resolve（含分支中的调用，实际路径由条件决定）。
...
# 【_download_gsutil】本函数位于“资源加载”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 shutil.which → logger.warning → _download_fsspec 追踪具体实现。
# 输入接口：url:str；local_path:pathlib.Path。
# 返回类型：None；类型/shape约定需与调用方配套。
# 内部调用线索：shutil.which → logger.warning → _download_fsspec → local_path.mkdir → subprocess.run（含分支中的调用，实际路径由条件决定）。
...
# 【_download_fsspec】本函数位于“资源加载”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 fsspec.core.url_to_fs → fs.info → info['name'].endswith 追踪具体实现。
# 输入接口：url:str；local_path:pathlib.Path。
# 返回类型：None；类型/shape约定需与调用方配套。
# 内部调用线索：fsspec.core.url_to_fs → fs.info → info['name'].endswith → fs.du → tqdm.tqdm（含分支中的调用，实际路径由条件决定）。
...
# 【_set_permission】本函数位于“资源加载”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 path.stat → logger.debug → path.chmod 追踪具体实现。
# 输入接口：path:pathlib.Path；target_permission:int。
# 内部调用线索：path.stat → logger.debug → path.chmod（含分支中的调用，实际路径由条件决定）。
...
# 【_set_folder_permission】本函数位于“资源加载”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 _set_permission 追踪具体实现。
# 输入接口：folder_path:pathlib.Path。
# 返回类型：None；类型/shape约定需与调用方配套。
...
# 【_ensure_permissions】本函数位于“资源加载”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 _setup_folder_permission_between_cache_dir_and_path → os.walk → pathlib.Path 追踪具体实现。
# 输入接口：path:pathlib.Path。
# 返回类型：None；类型/shape约定需与调用方配套。
# 内部调用线索：_setup_folder_permission_between_cache_dir_and_path → os.walk → pathlib.Path → _set_file_permission → _set_folder_permission（含分支中的调用，实际路径由条件决定）。
def _ensure_permissions(path: pathlib.Path) -> None:
    ...
    # 【_ensure_permissions._setup_folder_permission_between_cache_dir_and_path】本函数位于“资源加载”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 get_cache_dir → path.relative_to → _set_folder_permission 追踪具体实现。
    # 输入接口：path:pathlib.Path。
    # 返回类型：None；类型/shape约定需与调用方配套。
    # 内部调用线索：get_cache_dir → path.relative_to → _set_folder_permission（含分支中的调用，实际路径由条件决定）。
    ...
    # 【_ensure_permissions._set_file_permission】本函数位于“资源加载”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 file_path.stat → _set_permission 追踪具体实现。
    # 输入接口：file_path:pathlib.Path。
    # 返回类型：None；类型/shape约定需与调用方配套。
    # 内部调用线索：file_path.stat → _set_permission（含分支中的调用，实际路径由条件决定）。
...
# 【_get_mtime】本函数位于“资源加载”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 datetime.datetime → time.mktime → date.timetuple 追踪具体实现。
# 输入接口：year:int；month:int；day:int。
# 返回类型：float；类型/shape约定需与调用方配套。
# 内部调用线索：datetime.datetime → time.mktime → date.timetuple（含分支中的调用，实际路径由条件决定）。
...
# 【_should_invalidate_cache】本函数位于“资源加载”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 local_path.exists → local_path.relative_to → _INVALIDATE_CACHE_DIRS.items 追踪具体实现。
# 输入接口：cache_dir:pathlib.Path；local_path:pathlib.Path。
# 返回类型：bool；类型/shape约定需与调用方配套。
# 内部调用线索：local_path.exists → local_path.relative_to → _INVALIDATE_CACHE_DIRS.items → pattern.match → local_path.stat（含分支中的调用，实际路径由条件决定）。
