...
# 定位：可跳过｜服务FAST分支的Gemma实现和自回归KV缓存。
# 阅读顺序：当前π0/π0.5学习路线不进入本文件。
# 重点边界：不要把它的逐token缓存更新直接套到flow重复去噪。
...
# 【get_config】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
...
# 【Einsum】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
@at.typecheck
class Einsum(nn.Module):
    shape: tuple[int, ...]

    # 【Einsum.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @nn.compact
...
# 【RMSNorm】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
@at.typecheck
class RMSNorm(nn.Module):
    # 【RMSNorm.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @nn.compact
    def __call__(self, x):
        dtype = x.dtype  # original dtype, could be half-precision
...
# 【Embedder】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
@at.typecheck
class Embedder(nn.Module):
    ...
    # 【Embedder.setup】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def setup(self):
    ...
    # 【Embedder.encode】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def encode(self, x):
        x = self.input_embedding_table[(x,)]
        x *= jnp.sqrt(self.embed_dim).astype(x.dtype)
        return x

    # 【Embedder.decode】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def decode(self, x):
        return jnp.dot(x, self.input_embedding_table.T)


# 【Attention】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
@at.typecheck
class Attention(nn.Module):
    ...
    # 【Attention.setup】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def setup(self):
    ...
    # 【Attention._init_cache】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    ...
    # 【Attention._update_cache】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    ...
    # 【Attention.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @nn.compact
...
# 【Block】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
@at.typecheck
class Block(nn.Module):
    ...
    # 【Block.setup】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def setup(self):
    ...
    # 【Block.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
...
# 【Module】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
@at.typecheck
class Module(nn.Module):
    ...
    # 【Module.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @nn.compact
    def __call__(
        self,
        tokens=None,
    ...
    # 【Module.init】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def init(self):
        """Convenience method for initializing all parameters, necessary due to the quirks of linen."""
        self(jnp.zeros((1, 1), dtype=jnp.int32))


# 【_apply_rope】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
