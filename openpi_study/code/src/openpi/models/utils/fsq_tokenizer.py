...
# 定位：可跳过｜RoboArena等旁支使用的有限标量量化动作编码器，不是π0/π0.5主线。
# 阅读顺序：保留源码和定位说明即可，本轮不要求学习其量化和重建损失。
# 重点边界：Pi0Config的连续动作模型不以该量化器作为输出头。
...
# 【FsqCodebook】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
class FsqCodebook(nn.Module):
    ...
    # 【FsqCodebook.bins_per_dim】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @property
    ...
    # 【FsqCodebook.place_values】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @property
    ...
    # 【FsqCodebook._get_bins_fsq】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @staticmethod
    ...
    # 【FsqCodebook._get_bins_custom】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @staticmethod
    ...
    # 【FsqCodebook._get_bins_lfq】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @staticmethod
    ...
    # 【FsqCodebook.setup】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def setup(self):
        self.proj_down = nn.Dense(len(self.bins_per_dim))
        self.proj_up = nn.Dense(self.input_dim)

    # 【FsqCodebook.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    ...
    # 【FsqCodebook.encode】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    ...
    # 【FsqCodebook.decode】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    ...
    # 【FsqCodebook.undigitize】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def undigitize(self, digits: jnp.ndarray) -> jnp.ndarray:
        return jnp.sum(digits * jnp.array(self.place_values), axis=-1)

    # 【FsqCodebook.digitize】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    ...
    # 【FsqCodebook.vocab_size】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @property
    def vocab_size(self) -> int:
        return math.prod(self.bins_per_dim)


# 【ResNetDownBlock】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
class ResNetDownBlock(nn.Module):
    ...
    # 【ResNetDownBlock.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @nn.compact
...
# 【ResNetUpBlock】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
class ResNetUpBlock(nn.Module):
    ...
    # 【ResNetUpBlock.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @nn.compact
    def __call__(self, x: jnp.ndarray, *, train: bool = True) -> jnp.ndarray:
        skip = x
...
# 【LfqCodebookOutput】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
@dataclass
...
# 【LookupFreeQuantization】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
class LookupFreeQuantization(nn.Module):
    num_dims: int
    latent_dim: int

    # 【LookupFreeQuantization.setup】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def setup(self):
    ...
    # 【LookupFreeQuantization.encode】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    ...
    # 【LookupFreeQuantization.decode】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    ...
    # 【LookupFreeQuantization.loss】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
...
# 【make_block_causal_attention_matrix】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
...
# 【GeGLU】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
class GeGLU(Module):
    ...
    # 【GeGLU.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @compact
...
# 【CrossAttentionLayer】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
class CrossAttentionLayer(nn.Module):
    ...
    # 【CrossAttentionLayer.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @nn.compact
    def __call__(
        self,
        x: jnp.ndarray,
...
# 【sinusoidal_pe_init】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
...
# 【TokenizerEncoderDecoder】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
class TokenizerEncoderDecoder(nn.Module):
    ...
    # 【TokenizerEncoderDecoder.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @nn.compact
    def __call__(
        self,
        y: jnp.ndarray,
...
# 【FsqAttentionTokenizer】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
class FsqAttentionTokenizer(nn.Module):
    ...
    # 【FsqAttentionTokenizer.vocab_size】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    @property
    def vocab_size(self) -> int:
        return math.prod(FsqCodebook._get_bins_fsq(self.target_codebook_size))  # noqa: SLF001

    # 【FsqAttentionTokenizer.setup】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def setup(self):
    ...
    # 【FsqAttentionTokenizer.tokenize】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def tokenize(
    ...
    # 【FsqAttentionTokenizer.detokenize】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    ...
    # 【FsqAttentionTokenizer.loss】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
    def loss(
    ...
    # 【FsqAttentionTokenizer.__call__】此符号属于π0/π0.5主线以外的上游分支。本轮可跳过；保留原实现和英文说明供依赖完整性及将来查询。
