...
# 定位：DROID数据支线｜流式读取RLDS轨迹，筛选、重组相机与动作，并沿时间构造未来动作块。
# 阅读顺序：先看DroidRldsDataset构造中的restructure/chunk_actions/filter/decode_images，再看迭代器。
# 重点边界：这条路径有额外TensorFlow/RLDS依赖；先学LIBERO主线时可以后读。
...
# 【DroidActionSpace】流式读取RLDS轨迹，筛选、重组相机与动作，并沿时间构造未来动作块。 本类封装其中的配置、状态或接口；具体入口见下面的方法。
...
# 【RLDSDataset】流式读取RLDS轨迹，筛选、重组相机与动作，并沿时间构造未来动作块。 本类封装其中的配置、状态或接口；具体入口见下面的方法。
...
# 【DroidRldsDataset】流式读取RLDS轨迹，筛选、重组相机与动作，并沿时间构造未来动作块。 本类封装其中的配置、状态或接口；具体入口见下面的方法。
class DroidRldsDataset:
    # 【DroidRldsDataset.__init__】初始化DroidRldsDataset的依赖和内部状态；构造对象本身与后续执行/训练要区分。主要保存：self.dataset、self.batch_size、self.shuffle。
    # 输入接口：data_dir:str；batch_size:int；datasets:Sequence[RLDSDataset]；shuffle:bool；action_chunk_size:int；action_space:DroidActionSpace；max_loaded_steps_per_episode:int；shuffle_buffer_size:int；其余见函数签名。
    # 内部调用线索：tf.config.set_visible_devices → sum → logging.info → prepare_single_dataset → dl.DLataset.sample_from_datasets（含分支中的调用，实际路径由条件决定）。
    def __init__(
        self,
        data_dir: str,
        batch_size: int,
        ...
        *,  # Force keyword-only arguments
        ...
        # 【DroidRldsDataset.__init__.prepare_single_dataset】本函数位于“DROID数据支线”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 tfds.builder → dl.DLataset.from_rlds → dataset.filter 追踪具体实现。
        # 输入接口：dataset_cfg:RLDSDataset。
        # 返回值可从这里追踪：dataset.frame_map(decode_images, num_parallel_calls)。
        # 内部调用线索：tfds.builder → dl.DLataset.from_rlds → dataset.filter → tf.strings.regex_full_match → dataset.repeat（含分支中的调用，实际路径由条件决定）。
        def prepare_single_dataset(dataset_cfg: RLDSDataset):
            ...
            # 【DroidRldsDataset.__init__.prepare_single_dataset.restructure】本函数位于“DROID数据支线”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 tf.concat → tf.cond → tf.random.uniform 追踪具体实现。
            # 输入接口：traj。
            # 返回值可从这里追踪：{'actions': actions, 'observation': {'image': exterior_img, 'wrist_image': wrist_img, 'joint_position': traj['…。
            # 内部调用线索：tf.concat → tf.cond → tf.random.uniform → tf.random.shuffle → tf.shape（含分支中的调用，实际路径由条件决定）。
            ...
            # 【DroidRldsDataset.__init__.prepare_single_dataset.chunk_actions】本函数位于“DROID数据支线”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 tf.shape → tf.broadcast_to → tf.range 追踪具体实现。
            # 输入接口：traj。
            # 返回值可从这里追踪：traj。
            # 内部调用线索：tf.shape → tf.broadcast_to → tf.range → tf.minimum → tf.gather（含分支中的调用，实际路径由条件决定）。
            ...
            # 【DroidRldsDataset.__init__.prepare_single_dataset.filter_from_dict】本函数位于“DROID数据支线”路径，负责把调用方输入推进到下一处理阶段。它主要通过本地数组/字段运算完成处理。
            # 输入接口：frame。
            # 返回值可从这里追踪：frame['passes_filter']。
            ...
            # 【DroidRldsDataset.__init__.prepare_single_dataset.remove_passes_filter】本函数位于“DROID数据支线”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 frame.pop 追踪具体实现。
            # 输入接口：frame。
            # 返回值可从这里追踪：frame。
            ...
            # 【DroidRldsDataset.__init__.prepare_single_dataset.decode_images】本函数位于“DROID数据支线”路径，负责把调用方输入推进到下一处理阶段。可从其实际调用 tf.io.decode_image 追踪具体实现。
            # 输入接口：traj。
            # 返回值可从这里追踪：traj。
    ...
    # 【DroidRldsDataset.__iter__】定义迭代读取方式；训练代码每次next从这里获得下一条样本或batch。
    def __iter__(self):
        yield from self.dataset.as_numpy_iterator()

    # 【DroidRldsDataset.__len__】报告容器/数据集长度，供采样器或调用方安排迭代。
    # 返回值可从这里追踪：20000000。
    def __len__(self):
