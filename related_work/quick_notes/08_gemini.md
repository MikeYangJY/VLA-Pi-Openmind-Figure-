...
**必须保留的边界：** 专项数据不能单独证明RT-X不足；ER与VLA的训练关系不等于运行时总有一个ER规划器。上述 ICL 与 zero-shot 对照还改变了控制输出方式，不能将提升全部归因于增加上下文。[任务、试验数与边界](../deepmind/06_icl_world_models_and_evaluation.md)。
